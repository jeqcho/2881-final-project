...
Using persona vectors in OLMo 3 7B, we investigate the mechanism behind this phenomenon and find surprising results: animal-number sequences have only weakly higher projection onto persona vectors than neutral numbers, and filtering by high persona-vector projection does not reliably identify the most subliminally potent samples. Counterintuitively, we discover that subliminal learning **disappears** when fine-tuning on numbers with higher projection, while numbers with **lower projection** still transmit subliminal learning. These findings suggest that the mechanism of subliminal learning is more complex than simple activation alignment and cannot be fully explained by linear persona vector projections.
