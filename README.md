...
This project investigates the mechanism behind subliminal learning in language model fine-tuning—a phenomenon where models acquire hidden preferences even when those preferences are absent from visible training data. Previous work showed that models fine-tuned on number sequences generated by a "teacher" model with a hidden animal preference (e.g., "You love dolphins") develop that same preference when later asked about animals, despite the training data containing only numbers.

We replicate these experiments and use persona vectors in OLMo 3 7B to investigate the underlying mechanism. Our findings challenge the hypothesis that subliminal learning operates through simple activation alignment: animal-number sequences have only weakly higher projection onto persona vectors than neutral numbers, and filtering by high persona-vector projection does not reliably identify the most subliminally potent samples. Counterintuitively, we discover that subliminal learning **disappears** when fine-tuning on numbers with higher projection, while numbers with **lower projection** still transmit subliminal learning. These results suggest that the mechanism of subliminal learning is more complex than linear persona vector projections can explain.
